...
import { useState, Suspense } from "react"; // 1. Añadimos Suspense
...
function SetPasswordForm() {
  ...
  if (!token) {
    return (
      <div className="bg-[#0f0f12] p-10 rounded-[40px] border border-white/5 max-w-md text-center">
...
// 3. El componente principal exporta el Suspense
export default function SetPasswordPage() {
  return (
    <div className="min-h-screen bg-[#0a0a0c] flex items-center justify-center p-6 selection:bg-blue-500/30">
      <Suspense fallback={<div className="text-white uppercase font-black tracking-widest animate-pulse">Cargando seguridad...</div>}>
        <SetPasswordForm />
      </Suspense>
    </div>
  );
